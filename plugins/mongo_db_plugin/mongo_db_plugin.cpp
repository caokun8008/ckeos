...
{
   if ( (a.act.account == N(eosio.token)) && (a.act.name == N(transfer)) ) {
   ...
   try {
      for( const auto& b : balances ) {
         ...
         if( !accounts.update_one( make_document( kvp( "name", b.first )), update.view() ) ) {
            EOS_ASSERT( false, chain::mongo_db_update_fail, "Failed to update the balances of account ${n}", ("n", b.first));
         }
      }
   }
   catch (...) {
