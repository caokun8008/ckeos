...
vector<fc::variant> read_only::get_blocks(const read_only::get_blocks_params& params) const {
   EOS_ASSERT(!params.lower_block_num.empty() && params.lower_block_num.size() <= 64 && !params.upper_block_num.empty() && params.upper_block_num.size() <= 64,
   ...
   for (uint64_t i = lower_block_num; i <= upper_block_num; i++) {
      ...
      } EOS_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${i}", ("i", i))
