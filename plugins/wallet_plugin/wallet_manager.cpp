...
void wallet_manager::create(const std::string& name, const std::string& walt_pin) {
   ...
   EOS_ASSERT(valid_waltpin(walt_pin), wallet_exception, "Invalid wallet password, the password length must be between 6 and 18 characters long");
...
void wallet_manager::change_password(const std::string& name, const std::string& password) {
   ...
   EOS_ASSERT(valid_waltpin(password), wallet_exception, "Invalid wallet password, the password length must be between 6 and 18 characters long");
...
fc::variant call( const std::string& url,
   ...
   try {
      ...
      return eosio::client::http::do_http_call( *cp, fc::variant(v), false, false );
   }
...
fc::variant wallet_manager::create_account(const ::create_account_params& p) {
   ...
   permission_name permission = p.creator == "eosio.cname" ? N(newacc) : config::active_name;
...
fc::variant wallet_manager::push_action(const ::push_action_params& p) {
   ...
   if( !p.args.empty() ) {
      ...
      } EOS_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data",p.args))
   }
