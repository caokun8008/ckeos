...
void betstar::bet( account_name participant, uint32_t star, asset quantity )
{
   ...
   eosio_assert( star < 5, "invalid star");
   eosio_assert( quantity.symbol == asset().symbol, "asset must be system token" );
   eosio_assert( (quantity.amount >= 1000000) && (quantity.amount <= 100000000), "must between 100...10000 tokens" );

   issues issuestable( _self, cfg.current_id );
   auto issue = issuestable.find( cfg.current_id );
   eosio_assert( (issue != issuestable.end()) && issue->is_ongoing, "the issue is not available" );
   ...
   eosio_assert( iparticipant == issue->participants.end(), "have already betted" );

   INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {participant,N(active)}, { participant, N(eosio.bspool), quantity, std::string("betstar") } );
...
void betstar::lot()
{
   ...
   for (auto p : issue->participants) {
      if (p.star == star) {
         ...
         award -= fee;
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio.bspool),N(active)}, { N(eosio.bspool), p.participant, award, std::string("betstar award") } );
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio.bspool),N(active)}, { N(eosio.bspool), N(betstar.fee), fee, std::string("betstar fee") } );
      }
   }
   if (!winners.empty()) {
...
{
   require_auth( N(eosio.config) );

   eosio_assert( fee <= 10000, "must between 0...10000" );
   cfg.fee = fee;
   configtable.set( cfg, _self );
}
...
void betstar::setjrate( uint32_t rate )
{
   ...
   eosio_assert( rate <= 10000, "must between 0...10000" );
...
void betstar::setbrate( uint32_t rate )
{
   ...
   eosio_assert( rate <= 20, "must between 0...20" );
   cfg.bonus_rate = rate;
   configtable.set( cfg, _self );
}
...
void betstar::clear( uint64_t id )
{
   ...
   eosio_assert( issue != issuestable.end(), "the issue is not available" );
   issuestable.erase( issue );

   if ( (id % 10) == 1 ) {
      terms termstable( _self, id );
      auto term = termstable.find( id );
      eosio_assert( term != termstable.end(), "the term is not available" );
   ...
   if (count == 9) {
      ...
      if (size > 0) {
         ...
         for (auto w : term->winners[count]) {
            INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio.bspool),N(active)}, { N(eosio.bspool), w, award, std::string("top award") } );
            INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio.bspool),N(active)}, { N(eosio.bspool), N(betstar.fee), fee, std::string("betstar fee") } );
         }
      }
      --count;
   ...
   for (std::vector<std::vector<account_name>>::size_type i = count; i != 0; --i) {
      ...
      if (size > 0) {
         ...
         for (auto w : term->winners[i]) {
            INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio.bspool),N(active)}, { N(eosio.bspool), w, award, std::to_string(i + 1) + " wins award" } );
            INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio.bspool),N(active)}, { N(eosio.bspool), N(betstar.fee), fee, std::string("betstar fee") } );
         }
      }
   }
...
{
   accounts accountstable( N(eosio.token), N(eosio.bspool) );

   auto jackpot = accountstable.find( asset(0).symbol.name() );
   eosio_assert( jackpot != accountstable.end(), "the account does not exist" );
...
EOSIO_ABI( eosio::betstar, (create)(bet)(lot)(setfee)(setjrate)(setbrate)(clear) )
