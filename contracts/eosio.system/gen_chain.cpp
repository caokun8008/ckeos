...
void system_contract::bidchain( uint64_t id, account_name bidder, asset price )
{
    ...
    eosio_assert( price.symbol == asset().symbol, "asset must be system token" );
    eosio_assert( price.amount >= base_price, "must greater than base price" );
    chainbids bidstable( _self, id );

    auto b_info = bidstable.find( id );
    eosio_assert( b_info != bidstable.end(), "the bid id does not exist" );
    eosio_assert( b_info->is_ongoing, "the bidding has closed" );
    /*if ( b_info == bidstable.end() ) {
        INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {bidder,N(active)},
                             { bidder, N(eosio.bchain), price, std::string("bid chain ") + std::to_string(id) } );
    ...
    if ( ibidder != b_info->bidders.end() ) {  //the account has already bidden
        ...
        eosio_assert( (price.amount - ibidder->price.amount) >= (min->price.amount / 100), "must increase bid by 1%" );
        bidstable.modify( b_info, 0, [&]( auto& b ) {
           ...
           INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {bidder,N(active)},
                             { bidder, N(eosio.bchain), price - ib->price, std::string("increase bid ") + std::to_string(id) } );
           ib->price = price;
    ...
    if (b_info->bidders.size() < _gstate.current_chain_quota) {
        ...
        bidstable.modify( b_info, 0, [&]( auto& b ) {
           ...
           eosio_assert( (price.amount - min->price.amount) >= (min->price.amount / 100), "must increase bid by 1%" );
           INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {bidder,N(active)},
                             { bidder, N(eosio.bchain), price, std::string("bid chain ") + std::to_string(id) } );
           INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio.bchain),N(active)},
                             { N(eosio.bchain), min->bidder, min->price, std::string("refund bid on chain auction ") + std::to_string(id) } );
           min->bidder = bidder;
           min->price = price;
           stable_sort(b.bidders.rbegin(), b.bidders.rend());
        });
...
{
    require_auth( _self );
    eosio_assert( quota <= 100, "must between 0...100" );
...
void system_contract::maybe_start_bid_chain() {
    ...
    if ((week_id % 100) == 0)
        last_week_id = week_id - 77;//48 for release; 77 for test; 95 for demo
    else 
